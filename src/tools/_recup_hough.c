...
void line_trace_v(SDL_Surface* input, double theta, double rho){
    size_t height = input->h, width = input->w;
    double x, y;
    size_t i, j;
    for (x = 0; x < width; x++){
...
void line_trace_h(SDL_Surface* input, double theta, double rho){
    size_t height = input->h, width = input->w;
    double x, y;
    size_t i, j;
    for (y = 0; y < height; y++){
...
void get_rect(SDL_Surface* input, unsigned int** thetalist_inf, unsigned int** thetalist_sup, int** rholist_inf, int** rholist_sup, size_t length_inf, size_t length_sup){
    ...
    line_trace(input, ((double) up.theta)*(M_PI/180), (double) up.rho);
    line_trace(input, ((double) down.theta)*(M_PI/180), (double) down.rho);
    line_trace(input, ((double) left.theta)*(M_PI/180), (double) left.rho);
    line_trace(input, ((double) right.theta)*(M_PI/180), (double) right.rho);
}
